...
export function ProjectCachedContextProvider({ children }: { children: any }) {
  const [cachedProject, setCachedProject] = useLocalStorage<IProject | null>(
    "active.project",
    null
  );
 ...
 */
export function ProjectTemporaryContextProvider({
  children,
}: {
  children: any;
}) {
  ...
  const context = useMemo(
    () => ({ project: project ?? null, setProject: setProject }),
    [project]
  );
