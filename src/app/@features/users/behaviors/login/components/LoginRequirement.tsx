...
function MustLoginErrorMessage({children}: { children: ReactNode }) {
  ...
  return (
    <FeatureRegistrationBoundary disabled onAttemptRegister={contextState => setRegistry(Object.keys(contextState.features.available))}>
      <div className={className} data-error-msg={errorMsg}>
        <header>
          <span>{errorMsg}</span>
        </header>
        {registry.map(i => <div key={i}>{i}</div>)}
        {children}
      </div>
 ...
 */
export function LoginRequirement({children, state = true}: { children: any, state?: boolean }) {
  const jwt              = useJwt();
  ...
  if (userIsLoggedIn !== shouldBeLoggedIn) {
    return state ? <MustLoginErrorMessage>{children}</MustLoginErrorMessage> : null;
  }
  return children;
}